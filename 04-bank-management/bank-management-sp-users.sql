...
BEGIN
	...
	BEGIN TRY
		...
		END
		ELSE BEGIN
			INSERT INTO [dbo].[Users](FirstName, LastName, Email, Phone, Address, RegistrationDate, Balance)
			VALUES(@FirstName, @LastName, @Email, @Phone, @Address, @RegistrationDate, @Balance)
			
			INSERT INTO @Result(ResultStatus, ResultMessage, OperationType, AffectedRecordId, OperationDateTime)
			SELECT 1, 'Successful inserting data', 'INSERT', (SELECT SCOPE_IDENTITY()), GETDATE()
	...
	SET NOCOUNT ON;
		...
		BEGIN TRY
			...
			BEGIN
				...
				INSERT INTO @Result(ResultStatus, ResultMessage, UserId, FirstName, LastName, Email, Phone, Address, RegistrationDate, Balance, OperationDateTime)
				SELECT 1, 'Data has been sucessfully inserted', UserId, FirstName, LastName, Email, Phone, Address, RegistrationDate, Balance,GETDATE()
				FROM Users
				WHERE UserId = @UserId
...
BEGIN
	SET NOCOUNT ON;
		...
		BEGIN TRY
			INSERT INTO @Result(ResultStatus, ResultMessage, UserId, FirstName, LastName, Email, Phone, Address, RegistrationDate, Balance, OperationDateTime)
			SELECT 1, 'User found', UserId, FirstName, LastName, Email, Phone, Address, RegistrationDate, Balance, GETDATE()
			FROM Users
		END TRY
		BEGIN CATCH
			DECLARE @ErrorMessage NVARCHAR(4000)
...
INSERT INTO Users (FirstName, LastName, Email, Phone, Address, RegistrationDate, Balance) 
VALUES ('Delle', 'Seyah', 'delle@example.com', '123-123-1234', 'OldTown 123', '20240606', 7100),
			('Hannah', 'Joh-K', 'hannah@example.com', '123-123-1234', 'OldTown 456', '20240606', 1100),
			('John', 'Andras', 'john@example.com', '123-123-1234', 'OldTown 789', '20240606', 6100),
			('Kelly', 'Perez', 'keylly@example.com', '123-123-1234', 'OldTown 101', '20240606', 2100),
			('Gerad', 'Fox', 'gerad@example.com', '123-123-1234', 'OldTown 111', '20240606', 600)
**/